...
public class DemoApplication {
	...
	enum VehicleActions {
		...
		vehicleLocked,
		vehicleUnlocked,
		preConditioningEnded,
		preConditioningStarted
	};
	...
	private static String generatePayload(Integer recordCount) throws JsonProcessingException {
		...
		for (int i = 0; i < notifications.length; i++) {
			...
			dynamicData.put(VehicleActions.doorClose, new VehiclePayload(formattedDateTime, true));
			...
			KStream<String, String> processedStream = source.mapValues(value -> {
				...
				try {
					...
					List<Samples> samples = Arrays.asList(new Samples(Instant.now().toEpochMilli(), "DATA_STRING",
							notifications.get(0).current.alerts().alertEvent().get(VehicleActions.doorClose).value));
